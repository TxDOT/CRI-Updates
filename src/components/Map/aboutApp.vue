<template>
    ...
    <v-dialog v-model="aboutClick" max-width="500" persistent style="overflow-y: hidden;">
        <v-card id="finalCheck" v-model="aboutClick" min-height="540">
            ...
            <div>
                <v-card-text style="position: relative; color: black; bottom:0.5rem; left: 0%; text-align: left;">The County Road Inventory Map is a tool for county officials to verify and make updates to TxDOT’s 
            ...
            <v-divider style="position: relative; bottom: .5rem; color:black;"></v-divider>
            ...
            <div style="position: relative; left: 4rem; top: 2rem; padding-bottom: .5rem;">
                <v-card-text style="position:absolute; color: black; bottom: 0rem; right: 3.6rem; font-size: 10px; text-align: left;"><u>Build Version: 1.5.4</u></v-card-text>
                <v-btn style="position: relative; bottom: 0rem; left: 8rem; border:1px solid black;" underline tile color="#14375A" outlined @click="close(); clearEditBtn=false">
                    <u>close</u>
                </v-btn>
            </div>
...
export default {
    ...
    data (){
      return {
        updateList:['Added Upload/Download Inventory','Added Download Road Log','Added Geometry Checks (Self-Intersection, Minimum geometry length)', 'Updated sign-up URL','Added link to training videos','Adjusted Action item Body text font', 'Disabled mile info after ending asset mileage is completed', 
