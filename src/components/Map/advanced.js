...
function serverResponse(submitid){
    ...
    // let dataReturn = fetch(`https://testportal.txdot.gov/fmejobsubmitter/TPP/TPP_DEV_CRI_QAQC_AssetsRemoveDEV.fmw?SUBMIT_ID=${submitid}&USERNAME=${store.getters.getUserName}&EMAIL=${store.getters.getUserEmail}&opt_showresult=false&opt_servicemode=sync`, {headers:{'Authorization':'fmetoken token=a264041e806b7199db0384c0357c2bc08b882750', 'Content-Type': 'text/plain'}})
    let dataReturn = fetch(`https://testportal.txdot.gov/fmejobsubmitter/TPP/${criConstants.uploadDataWebhookName}?SUBMIT_ID=${submitid}&USERNAME=${store.getters.getUserName}&EMAIL=${store.getters.getUserEmail}&opt_showresult=false&opt_servicemode=sync`, {headers:{'Authorization':`fmetoken token=${criConstants.uploadDataWebhookToken}`, 'Content-Type': 'text/plain'}})
